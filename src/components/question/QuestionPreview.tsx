    ...
    <div className="rounded-xl bg-secondary p-4">
      ...
      <div className="mt-2 rounded-xl bg-card p-3">
        ...
        <div className="mt-3 space-y-2">
          {isChoice ? (
            <div className="space-y-2">
              {(question.options ?? []).slice(0, 6).map((o) => (
                <div
                  key={`${o.value}:${o.label}`}
                  className="rounded-lg border border-border bg-control px-3 py-2 text-sm"
                >
                  {o.label || o.value}
                </div>
              ))}
          ...
          {isNumber ? (
            <div className="rounded-lg border border-border bg-control px-3 py-2 text-sm text-secondary-foreground/80">
          ...
          {question.answer_type === 'boolean' ? (
            <div className="grid grid-cols-2 gap-2">
              <div className="rounded-lg border border-border bg-control px-3 py-2 text-sm">Да</div>
              <div className="rounded-lg border border-border bg-control px-3 py-2 text-sm">Нет</div>
            </div>
          ) : null}
        </div>

        <div className="mt-3 flex flex-wrap gap-2 text-[11px] text-secondary-foreground/70">
          <span className="rounded-md border border-border bg-control px-2 py-1">key: {question.key || '—'}</span>
          <span className="rounded-md border border-border bg-control px-2 py-1">order: {question.order ?? 0}</span>
          <span className="rounded-md border border-border bg-control px-2 py-1">
